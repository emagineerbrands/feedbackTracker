...
export class CallLogsSelectFilterComponent implements IFilterAngularComp {
  ...
  agInit(params: IFilterParams): void {
    ...
    const field = params.colDef.field; // Store the field value to avoid repetitive access
    ...
    if (fieldMapping) {
        if (fieldMapping.customMapping) {
            ...
            //const dataSource = this.internalService.dropdownData[fieldMapping.dataSource];
  ...
  doesFilterPass(params: IDoesFilterPassParams): boolean {
    const field = this.params.colDef.field; // e.g., 'pickUp'

    if (!field) {
        return false;
    }

    const fieldMapping:any = null;
