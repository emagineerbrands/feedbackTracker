...
@Component({
  ...
  styleUrl: './shipping-tracker.component.css'
})
export class ShippingTrackerComponent {
  ...
  searchOrderById(trackId: number){
  ...
  onSubmit() {
    ...
    this.returnsTracker.quantityReturnedInTotal = this.returnsTracker.skuGroup.reduce((total:number, skuItem: { qtyReturned: any; }) => total + Number(skuItem.qtyReturned), 0);
    this.returnsTracker.quantityDamagedInTotal = this.returnsTracker.skuGroup.reduce((total:number, skuItem: { qtyDamaged: any; }) => total + (Number(skuItem.qtyDamaged) || 0), 0);
    const authUser = JSON.parse(localStorage.getItem('authDetails') || 'null');
    const jsonData: any = {
